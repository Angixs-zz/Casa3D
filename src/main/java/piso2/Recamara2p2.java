...
public class Recamara2p2 {
    ...
    private static void dibujarCloset(float xMinGeo, float zMinGeo, float xMaxGeo, float zMaxGeo) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            Cubo.dibujar(
                    offsetXPpuerta + (i % 2 == 0 ? anchoPuerta * 0.3f : -anchoPuerta * 0.3f), // Manijas encontradas en
                                                                                              // el centro de cada par
    ...
    private static void dibujarEstante(float xMinGeo, float zMinGeo, float xMaxGeo, float zMaxGeo) {
        ...
        // Para simplificar y hacerlo realista, pondremos las repisas sobre un fondo
        // oscuro
        Cubo.dibujar(0f, alto / 2f, largo / 2f + 0.005f, ancho * 0.9f, alto * 0.95f, 0.01f, 0.2f, 0.1f, 0.05f); // Fondo
                                                                                                                // interior
    ...
    private static void dibujarBuroConTV(float xMinGeo, float zMinGeo, float xMaxGeo, float zMaxGeo) {
        ...
        // La pared está en X positivo (derecha). La TV debe colgar mirando hacia la
        // izquierda (-X).
        float altoTV = 0.8f;
        ...
        Cubo.dibujar(ancho / 2f - grosorTV - 0.005f, yCentroTV, 0f, 0.01f, altoTV * 0.9f, anchoPantallaTV * 0.95f, 0.1f,
                0.1f, 0.15f); // Pantalla apagada / negra
    ...
    private static void dibujarCama(float xMinGeo, float zMinGeo, float xMaxGeo, float zMaxGeo) {
        ...
        Cubo.dibujar(0f, altoBase + altoColchon / 2f, 0f, ancho * 0.98f, altoColchon, largo * 0.98f, 0.95f, 0.95f,
                0.9f);

        // Edredón / Cubrecama (Azul marino como en la imagen)
    ...
    private static void dibujarBuroLateral(float xMinGeo, float zMinGeo, float xMaxGeo, float zMaxGeo) {
        ...
        // Como la cama está a lo largo del eje X (pies hacia -X), los cajones deberían
